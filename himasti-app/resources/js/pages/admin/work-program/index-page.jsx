...
export default function WorkProgramIndexPage() {
    const { workPrograms, filters, divisions, statuses, sortableItems } = usePage().props;
        ...
        <AppLayout>
            <div className="space-y-6">
                <AdminPageHeader
                    ...
                    action={
                        <Button asChild>
                            <Link href={route("admin.work-programs.create")}>Tambah Program Kerja</Link>
                        </Button>
                    }
                />
                ...
                >
                    <form onSubmit={submitFilter} className="grid gap-3 md:grid-cols-2 xl:grid-cols-6">
                        <Input
                        ...
                        <Select value={data.is_published || "all"} onValueChange={(value) => setData("is_published", value === "all" ? "" : value)}>
                            <SelectTrigger><SelectValue placeholder="Semua visibilitas" /></SelectTrigger>
                            <SelectContent>
                ...
                <SortableOrderPanel
                    ...
                    items={sortableItems.map((item) => ({
                        ...
                        imageAlt: item.name,
                        subtitle: [item.division?.name, item.year].filter(Boolean).join(" · "),
                        meta: item.slug,
                        statusLabel: item.is_published ? "Ditampilkan" : "Disembunyikan",
                            ...
                            <>
                                        ...
                                        <TableBody>
                                            {workPrograms.data.map((item) => (
                                                <TableRow key={item.id}>
                                                    ...
                                                    <TableCell>
                                                        <StatusBadge
                                                            label={item.is_published ? "Ditampilkan" : "Disembunyikan"}
                                                            tone={item.is_published ? "published" : "draft"}
                                                        />
                                                    </TableCell>
                                                    ...
                                                    <TableCell className="text-right">
                                                        <div className="flex justify-end gap-2">
                                                            <Button variant="outline" size="sm" asChild>
                                                                <Link href={route("admin.work-programs.edit", item.id)}>Ubah</Link>
                                                            </Button>
                                                            <DeleteConfirmationDialog
                                                                ...
                                                                onConfirm={() => router.delete(route("admin.work-programs.destroy", item.id))}
                                                            />
                                                        </div>
                                                    </TableCell>
                                                </TableRow>
                                ...
                                <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                                    <p className="text-sm text-muted-foreground">
                                        Menampilkan {workPrograms.from} - {workPrograms.to} dari {workPrograms.total} data.
                                    </p>
