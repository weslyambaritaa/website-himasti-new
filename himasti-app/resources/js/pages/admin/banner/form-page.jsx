...
export default function BannerFormPage() {
    ...
    const submit = (event) => {
        ...
        post(isEdit ? route("admin.banner.update", banner.id) : route("admin.banner.store"));
    };
                ...
                <form onSubmit={submit} className="space-y-6">
                    <FormSection title="Informasi Banner">
                        ...
                        <div className="grid gap-2">
                            ...
                            <Textarea id="subtitle" rows={4} value={data.subtitle} onChange={(event) => setData("subtitle", event.target.value)} />
                            {errors.subtitle ? <p className="text-sm text-red-500">{errors.subtitle}</p> : null}
                        </div>
                    </FormSection>
                    <FormSection title="Media" description="Unggah gambar banner dengan ukuran proporsional agar tetap nyaman di desktop dan mobile.">
                        <div className="grid gap-4 lg:grid-cols-[1fr_280px]">
                            ...
                            <MediaPreview file={data.image} fallbackPath={banner?.image} alt={banner?.title ?? "Banner"} emptyLabel="Belum ada gambar banner" />
                        </div>
                    </FormSection>
                    ...
                    <FormActions cancelHref={route("admin.banner.index")} processing={processing} />
                </form>
            </div>
        </AppLayout>
    );
...
function ButtonBack() {
    return (
        <Link href={route("admin.banner.index")} className="inline-flex h-9 items-center rounded-md border px-4 text-sm">
            Kembali
        </Link>
    );
}
